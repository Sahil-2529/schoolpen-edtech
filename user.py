...
class Lesson:
    def __init__(self, title, objective, content, application, relevance_to_subject, skill_gained, events_problem, career_path, user_id, book_id):
...
class Question:
    def __init__(self, question_type, question_level, question_text, answer_type, options, lesson_id):
        self.id = str(ObjectId())  # Generate a new string-based ID
...
class LessonSchema(ma.Schema):
    class Meta:
        fields = ('_id', 'title', 'objective', 'content', 'application', 'relevance_to_subject', 'skill_gained', 'events_problem', 'career_path', 'started', 'last_interacted_with', 'user_id', 'book_id', 'questions', 'completed')
...
def add_lesson():
    ...
    lesson._id = str(lesson_id)  # Convert ObjectId to string before returning to the frontend
